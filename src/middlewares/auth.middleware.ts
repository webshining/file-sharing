...
export default async (req: any, res: Response, next: NextFunction) => {
	...
	if (!authorization || !authorization.startsWith("Bearer")) return res.status(401).json({ detail: "Unauthorized" });
	const accessData = await authService.tokenDecode(authorization.split(" ")[1]);
	if (!accessData) return res.status(401).json({ detail: "Unauthorized" });
